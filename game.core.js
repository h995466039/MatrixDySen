...
const qaNormalMode = urlParams.get('qa') === 'normal';
...
function makeStellarProject(savedState = null) {
  ...
  const components = Array.isArray(savedState?.components)
    ? savedState.components.filter(component => component && component.type).map(component => ({
      ...
      maxLifetime: Math.max(1, Number(component.maxLifetime) || 900)
    }))
    : [];
  return {
...
function terrainAt(cell, planetId = null) {
  ...
  const currentPlanet = planetId || terrainPlanetId;
...
function readSave() {
  try {
    ...
    terrainPlanetId = planetById[saved.activePlanet] ? saved.activePlanet : 'home';
...
terrainPlanetId = state.activePlanet;
{
